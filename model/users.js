...
userDb.login = (userData) => {
  return userCollection.getUserCollection().findOne({ email: userData.email }).then((user) => {
    if (!user) return 4;
    return bcrypt
      .compare(userData.password, user.password)
      .then((dataAuthentication) => {
...
userDb.newPassword = (password) => {
  return userCollection.getResetTokenCollection().then((model) => {
    return model
      .findOne({ resettoken: password.resetToken })
      .then((userToken) => {
        ...
        return userCollection.getUserCollection().then((model1) => {
          return model1.findOne({ _id: userToken._userId }).then((userData) => {
            ...
            return userDb
              .generatePassword(password.newPassword)
